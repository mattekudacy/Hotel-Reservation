...
//palitan yung connect ng pangalan ng connection na program
//RoomForm ata eto nung una, pinalitan ko lang pangalan kasi nag eerror
public class RoomsForm extends connect {
    ...
    //para to sa table
    //palitan nalang yung variables depende sa connection na program
    // id is yung room id
    //rt is yung roomtype
    //rc is yung room capacity
    //cid is yung check in date
    //cod is yung check out
    public void Select(){
    String [] columnNames = {"ROOMNUMBER"," ROOMTYPE", "STATUS"};
    tbModel.setColumnIdentifiers(columnNames);
    int [] roomNumbers ={101,102,103,104,105,106,107,108,109,110,111,112,201,202,203,204,205,206,207,208,301,302,303,304,305,401,402,403,501,502};
    try{
        for(int i = 0; i < roomNumbers.length; i++){
            String status = "VACANT";
            String roomType = "";
            int roomNo = roomNumbers[i];
            if(roomNo < 113){
                roomType = "Deluxe";
            }
            else if ((roomNo < 209) && (roomNo > 112)){
                roomType = "Premium";
            }
            else if ((roomNo < 306) && (roomNo > 208)){
                roomType = "Executive";
            }
            else if ((roomNo < 404) && (roomNo > 305)){
                roomType = "Diplomatic";
            }
            else if ((roomNo < 503) && (roomNo > 403)){
                roomType = "Presidential";
            }
            while(rs.next()){
                int dataBaseRoomNo = rs.getInt("ROOMNO");
                if(roomNo == dataBaseRoomNo){
                    status = "OCCUPIED";
                }
            }
            jTable1.setVisible(true);
            tbModel.addRow(new Object [] {roomNo,roomType, status});
            x++;
            Refresh_RS_STMT();
        }
//        while(rs.next()){
...
////            jTable1.setVisible(true);
////            tbModel.addRow(new Object []{rn,rt,});
//            
//        }
        Refresh_RS_STMT();
    }catch(SQLException err){
            JOptionPane.showMessageDialog(RoomsForm.this, err.getMessage());
       }
}
    void showDate(){
         Date d = new Date();
    ...
    private void initComponents() {
        ...
        roomsbg = new javax.swing.JLabel();
        ...
        jScrollPane2.setBounds(10, 50, 700, 320);
        ...
        roomsbg.setIcon(new javax.swing.ImageIcon(getClass().getResource("/HotelReservationSystem/img/dashboardbg.png"))); // NOI18N
        roomsbg.addMouseMotionListener(new java.awt.event.MouseMotionAdapter() {
            public void mouseMoved(java.awt.event.MouseEvent evt) {
                roomsbgMouseMoved(evt);
            }
        });
        getContentPane().add(roomsbg, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, -1, -1));
    ...
    private void roomsbgMouseMoved(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_roomsbgMouseMoved
    ...
    }//GEN-LAST:event_roomsbgMouseMoved
    ...
    private javax.swing.JLabel roomsbg;
