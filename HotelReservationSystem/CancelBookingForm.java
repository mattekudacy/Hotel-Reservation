...
public class CancelBookingForm extends ConnectRoomBookingandCheckIn {
...
//        showData();
    ...
    public void Select(){
        String[] columnNames = {"Name", "Check in Date", "Check out Date", "Room Number", "Room Type", "Reserve Code"};
        tbModel.setColumnIdentifiers(columnNames);
        try{
            while(rsRoomBooking.next()){
                String reserveCodeAdd = rsRoomBooking.getString("RESERVECODE");
                String statusAdd = rsRoomBooking.getString("STATUS");
                if((reserveCodeAdd.equals("FROM WALK IN CHECKIN")) || (statusAdd.equals("CHECKEDIN"))){
                    //  && (statusAdd.equals("CHECKEDIN"))
                    System.out.println("skip");
                }
                else{ 
                    String nameAdd = rsRoomBooking.getString("NAME");
                    Date checkInDateAdd = rsRoomBooking.getDate("CHECKINDATE");
                    Date checkOutDateAdd = rsRoomBooking.getDate("CHECKOUTDATE");
                    int roomNumberAdd = rsRoomBooking.getInt("ROOMNO"); //rsRoomBooking.getString("NAME"); 
                    String roomTypeAdd = rsRoomBooking.getString("ROOMTYPE");
                    tbBooking.setVisible(true);
                    tbModel.addRow(new Object []{nameAdd, checkInDateAdd, checkOutDateAdd, roomNumberAdd, roomTypeAdd, reserveCodeAdd});
                
                }
            }
            Refresh_RSROOMBOOKING_STMT ();
        }catch(SQLException err){
            JOptionPane.showMessageDialog(CancelBookingForm.this, err.getMessage());
        }
    }
    ...
    private void initComponents() {
        ...
        cancelbookingbg = new javax.swing.JLabel();
        ...
        checkoutpane.add(jButton1);
        jButton1.setBounds(290, 320, 120, 25);

        tbBooking.setModel(tbModel);
        jScrollPane1.setViewportView(tbBooking);

        checkoutpane.add(jScrollPane1);
        jScrollPane1.setBounds(10, 42, 700, 270);
        ...
        cancelbookingbg.setIcon(new javax.swing.ImageIcon(getClass().getResource("/HotelReservationSystem/img/dashboardbg.png"))); // NOI18N
        cancelbookingbg.addMouseMotionListener(new java.awt.event.MouseMotionAdapter() {
            public void mouseMoved(java.awt.event.MouseEvent evt) {
                cancelbookingbgMouseMoved(evt);
            }
        });
        getContentPane().add(cancelbookingbg, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, -1, -1));
    ...
    CancelBookingForm.this.dispose();
    ...
    private void cancelbookingbgMouseMoved(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_cancelbookingbgMouseMoved
    ...
    }//GEN-LAST:event_cancelbookingbgMouseMoved
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        Refresh_RSROOMBOOKING_STMT();
        int response = JOptionPane.showConfirmDialog(this, "Do you wish to cancel a booking?","Cancel Booking",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
        switch (response) {
            case JOptionPane.YES_OPTION:
                try{
                    String code;
                    code = JOptionPane.showInputDialog("Please enter the reservation code of the booking that you wish to cancel ");
                    if(code == null){
                        Refresh_RSROOMBOOKING_STMT ();
                    }
                    int iteratorcheck = 0;
                    while(rsRoomBooking.next()){
                        ...
                        String reserveCode = rsRoomBooking.getString("RESERVECODE");
                        String status = rsRoomBooking.getString("STATUS");
                        if(code.equals(reserveCode)){
                            System.out.println("same code");
                            int result = JOptionPane.showConfirmDialog(null, "Are you sure you want to delete reservation: " + reserveCode + "?", "Confirmation Message", JOptionPane.YES_OPTION);
                        ...
                        if(lastchecker){
                            JOptionPane.showMessageDialog(null,"No records for your reservation code has been found");
                            Refresh_RSROOMBOOKING_STMT ();
                            break;
                        }
                    }
                    Refresh_RSROOMBOOKING_STMT();
                    if(iteratorcheck == 0){
                        JOptionPane.showMessageDialog(null, "No Booking Records");
                    }
                }catch(SQLException err){
                    JOptionPane.showMessageDialog(CancelBookingForm.this, err.getMessage());
                }
            ...
            setDefaultCloseOperation(CancelBookingForm.DO_NOTHING_ON_CLOSE);
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new CancelBookingForm().setVisible(true);
            }
        });
    }
    ...
    private javax.swing.JLabel cancelbookingbg;
